...
window.addEventListener('DOMContentLoaded', () => {
  initSmoothScroll();   // Awwwards feel Lenis
  initNavigation();     // Navigation links click mapping
  initSpaceBackground(); // Celestial background rendering
  initCustomCursor();   // 60FPS Follower cursor
  initMagneticButtons(); // Interactive magnet pulls
  initHero3D();         // Hero scroll exploding assembly
  initBox3D();          // 3D Customizer box viewport
  initCalculator();     // Form connecting to 3D and dieline
  initScrollReveals();  // Typography scroll fade reveals
});
